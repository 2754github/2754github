...
Web 系エンジニアです。（FE, BE, 時々インフラ）
現在はフリーランスで活動しています。（お仕事の依頼は fkesys@gmail.com まで 🙇‍♂️）

## 普段やっていること

プロジェクトを`企画` `設計` `実装` `テスト` `運用`と分けた場合の、`設計` `実装`の部分をメインに行っています。
また、[**MENTA**](https://menta.work/plan/2019)にて初学者の方向けのメンターとしても活動しています。
（自分が開発するだけでなく、`日本のIT力`の底上げにも寄与したいと考えているため。）
